...
% gridSpacing   -spacing for sampling抽样 dense descriptors
...
for ii = 1:length(subfolders),
    ...
    if ~strcmp(subname, '.') & ~strcmp(subname, '..'),
        database.nclass = database.nclass + 1;      %图像库中，图像总类数
        
        database.cname{database.nclass} = subname;  %图像类别名称(即子目录名，就中类名)
        
        frames = dir(fullfile(rt_img_dir, subname, '*.jpg'));
        
        c_num = length(frames);    %当前类，图像总数       
        database.imnum = database.imnum + c_num;   %图像库中，图像总数
        database.label = [database.label; ones(c_num, 1)*database.nclass];   %当类图像的标号
        
        siftpath = fullfile(rt_data_dir, subname);        
        if ~isdir(siftpath),
            mkdir(siftpath);    %娄当前类，建立一个目录，用于存它的特征
        end;
        ...
        for jj = 1:c_num,
            ...
            [im_h, im_w] = size(I);
            %图像的高或，最大不能大于300（maxImSize）
            if max(im_h, im_w) > maxImSize,
                I = imresize(I, maxImSize/max(im_h, im_w), 'bicubic');
                [im_h, im_w] = size(I);
            end;
            
            % 网格SIFT稠密采样 make grid sampling SIFT descriptors
            ...
            save(fpath, 'feaSet');
            database.path = [database.path, fpath];  %把此图的Mat文件名也存起来
        end;    
    end;
end;
